...
export const HeaderPresenter: FC<THeaderPresenterProps> = ({
  state: { isMobile, locale, language, menuList },
          ...
          <div className="hidden md:block">
            <nav className="flex-wrap justify-center items-center px-2 text-base md:px-0">
              {menuList.map((item: TMenuItem, inddex: number) => {
                return (
                  <Link key={inddex} href={item.path}>
                    <a className="inline-block p-3 hover:text-gray-900">
                      <span className="hidden mr-3 md:inline-block">{item.icon}</span>
...
export const Header: FC = () => {
  ...
  return (
    <HeaderPresenter
      ...
      actions={{ onMenuButtonClick: handleOverlayMenuDisplay }}
    />
  )
}
