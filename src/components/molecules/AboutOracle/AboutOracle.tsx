...
export type TAboutOraclePresenterProps = {
  state: {
    locale: TLocale
    language: TLanguage
  }
  actions: {
    onAvatarClick: (argument: TAvatarLink) => void
  }
}

export const AboutOraclePresenter: FC<TAboutOraclePresenterProps> = ({
  state: { language },
    ...
    <div className="flex flex-col mx-auto mt-24 lg:flex-row-reverse lg:w-4/5">
      ...
      <div className="w-full lg:pr-10 lg:mt-0 lg:w-1/2">
        ...
        <h1 className="mt-4 mb-2 text-3xl font-bold text-gray-900 lg:text-5xl">
          {language.page.aboutPage.section.oracle.name}
        </h1>
        <p className="mt-6 leading-8">{language.page.aboutPage.section.oracle.description}</p>
          ...
          <button
            onClick={() => {
              onAvatarClick({
                ...
                bearName: 'oracle',
                nftId: 'A0004',
                link: 'https://opensea.io/assets/0x495f947276749ce646f68ac8c248420045cb7b5e/85612792707671471240665121138616411211534411599642434463980733058434161704961'
              })
              //
            }}
          ...
          <button
            onClick={() => {
              onAvatarClick({
                ...
                bearName: 'oracle',
                nftId: 'A0021',
                link: 'https://opensea.io/assets/matic/0x2953399124f0cbb46d2cbacd8a89cf0599974963/85612792707671471240665121138616411211534411599642434463980733083722929143809'
              })
              //
            }}
            className="block overflow-hidden -ml-4 w-24 h-24 rounded-full"
          ...
          <button
            onClick={() => {
              onAvatarClick({
                ...
                bearName: 'oracle',
                nftId: 'A0075',
                link: 'https://opensea.io/assets/matic/0x2953399124f0cbb46d2cbacd8a89cf0599974963/85612792707671471240665121138616411211534411599642434463980733144196068671489'
              })
              //
            }}
            className="block overflow-hidden -ml-4 w-24 h-24 rounded-full"
          ...
          <button
            onClick={() => {
              onAvatarClick({
                ...
                bearName: 'oracle',
                nftId: 'A0111',
                link: 'https://opensea.io/assets/matic/0x2953399124f0cbb46d2cbacd8a89cf0599974963/85612792707671471240665121138616411211534411599642434463980733183778487271425'
              })
              //
            }}
            className="block overflow-hidden -ml-4 w-24 h-24 rounded-full"
          ...
          <button
            onClick={() => {
              onAvatarClick({
                ...
                bearName: 'oracle',
                nftId: '',
                link: 'https://opensea.io/assets/bit-bear-club-avatar-series?search[stringTraits][0][name]=Name&search[stringTraits][0][values][0]=Oracle&search[sortAscending]=true&search[sortBy]=PRICE'
              })
              //
            }}
...
export const AboutOracle: FC = () => {
  const {
    state: { language, locale }
  } = Hooks.Locale.useLocaleContext()

  const onAvatarClick = useCallback((argument: TAvatarLink) => {
    window.location.href = argument.link
  }, [])

  return <AboutOraclePresenter state={{ language, locale }} actions={{ onAvatarClick }} />
}
