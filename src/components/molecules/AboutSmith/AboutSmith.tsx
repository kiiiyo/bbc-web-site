...
export type TAboutSmithPresenterProps = {
  state: {
    locale: TLocale
    language: TLanguage
  }
...
export const AboutSmithPresenter: FC<TAboutSmithPresenterProps> = ({
  state: { language },
    ...
    <div className="flex flex-col mx-auto mt-24 lg:flex-row lg:w-4/5">
      ...
      <div className="w-full lg:pl-10 lg:mt-0 lg:w-1/2">
        ...
        <h1 className="mt-4 mb-2 text-3xl font-bold text-gray-900 lg:text-5xl">
          {language.page.aboutPage.section.smith.name}
        </h1>
        <p className="mt-6 leading-8">{language.page.aboutPage.section.smith.description}</p>
          ...
          <button
            onClick={() => {
              onAvatarClick({
                ...
                bearName: 'smith',
                nftId: '',
                link: 'https://opensea.io/assets/bit-bear-club-avatar-series?search[stringTraits][0][name]=Name&search[stringTraits][0][values][0]=Smith&search[sortAscending]=true&search[sortBy]=PRICE'
              })
              //
            }}
...
export const AboutSmith: FC = () => {
  ...
  return <AboutSmithPresenter state={{ language, locale }} actions={{ onAvatarClick }} />
}
